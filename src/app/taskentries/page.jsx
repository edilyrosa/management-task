...
import TaskEntryForm from './TaskEntryForm';
import DeleteConfirmModal from '../components/DeleteConfirmModal';
import { useTaskEntries } from '../../context/TaskEntryContext';

export default function TaskEntryPage() {
  const { taskEntries, saveTaskEntry, deleteTaskEntry } = useTaskEntries();
  ...
  if (!taskEntries) {
    return <p className="text-gray-500">Loading task entries...</p>;
  }
  ...
  const handleSave = async (taskEntryData) => {
    try {
      await saveTaskEntry(taskEntryData);
      setIsFormOpen(false);
      setEditingTaskEntry(null);
    } catch (error) {
      console.error('❌ Error saving task entry:', error);
      alert(error.message || 'Error saving task entry');
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteTaskEntry(deleteTaskEntryId);
      setDeleteTaskEntryId(null);
      setDeleteTaskEntryName('');
    } catch (error) {
      console.error('❌ Error deleting task entry:', error);
      alert(error.message || 'Error deleting task entry');
    }
  };
